$(document).ready(function(){
	var querygrid = $("#gridview").jqGrid({
        colModel: [ {name: '報表編號',width: 60,align: "left"},
                    {name: '報表名稱',width: 110,align: "left"},
                    {name: '報表版本',width: 30,align: "center"},                    
                    {name: '上版日期',width: 55,align: "left"},
                    {name: '製表日期',width: 55,align: "left"},
                    {name: '製表人員姓名',width: 60,align: "center"},
                    {name: '是否匯出',width: 30,align: "center"},
					{header: '狀態',name: 'rptSignStatus',width: 55,align: "center"},
					{header: '申請日期',name: 'rptApplicationDate',width: 55,align: "left"},
					{header: '放行日期',name: 'rptApproveDate',width: 55,align: "left"}],
        ...
        onSelectRow: function(id) {
        	if(querygrid.getCell(id, '是否匯出')=='N'){
            	...
            	/*
            	$("#addDef").css("color","#005BA1");
            	$("#addDef").attr("disabled",false);
            	$("#modDef").css("color","#005BA1");
            	$("#modDef").attr("disabled",false);
            	*/
            	//不能刪除，可修改版次加一
	...
	qDialog.dialog({
    	...
    	buttons:API.createJSON([{
			...
			$("#gridview").clearGridData().addGridData([
				 ["","ACD01","總分類帳", "1", "2014-02-08", "2014-02-08","王大明","Y","已放行","2014-01-10 09:30","2014-01-16 14:30"],
				 ["","ACD01", "總分類帳","2", "2014-01-10", "2014-02-09","王大明","Y","未放行","2013-01-10 01:35",""],
				 ["","ACD10", "會計過帳明細表", "1", "2014-02-07", "2014-04-08","王大明","Y","已放行","2014-01-15 09:30","2014-01-15 11:30"],
				 ["","ACD10", "會計過帳明細表","2", "2014-02-12", "2014-02-09","王大明","N","已放行","2014-01-10 01:30","2014-01-12 14:30"] 
			]);
			}
	...
	//送審
    var applyDialog = $("#applyDialog");
	applyDialog.dialog({
    	height: 260,width: 500,modal: true,
    	buttons:API.createJSON([{
    		key:i18n.def.sure,value:function(){applyDialog.dialog('close');}
    	},{ key:i18n.def.close,value:function(){applyDialog.dialog('close');}
    	}])
    });
    ...
    eDialog.dialog({
    	height:800,width:950,modal:true
    	/*
    	buttons:API.createJSON([{
...
*/	
    ...
    $('#hiddenCodeData').jqGrid('addRowData', rowIndex, indexfields[rowIndex]);
		...
		$('input:radio[name=hiddenCode]').change(function(){				
				if($("input[name='hiddenCode']:checked").val() == 'Y'){
						$('#li4').show();
						$('#tabs-4').show();						
					}
				if($("input[name='hiddenCode']:checked").val() == 'N'){
						$('#li4').hide();
						$('#tabs-4').hide();
					}
			});
